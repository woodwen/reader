...
import com.woodnoisu.reader.feature.reader.presentation.page.model.PageMode
import com.woodnoisu.reader.feature.reader.presentation.page.model.PageStyle
import com.woodnoisu.reader.feature.reader.presentation.page.model.TxtPage
...
internal abstract class PageLoader(pageView: PageView,
                          collBook: ReaderBookDomainModel
){
    //协程作用域
    private val uiScope = CoroutineScope(Dispatchers.Main)

    // 书本对象
    protected val mCollBook: ReaderBookDomainModel = collBook
    ...
    // 当前章节列表
    protected val mChapterList: MutableList<ReaderChapterDomainModel> = ArrayList(1)
    ...
    // 下一章的页面列表缓存
    private val mNextPageList: MutableList<TxtPage> = ArrayList()
    ...
    // 存储阅读记录类
    protected lateinit var mBookRecord: ReaderRecordDomainModel
...
//    private var asyncTask: AsyncTask<Int, Void, MutableList<TxtPage>>? = null
     ...
     * 设置阅读记录
     */
    fun setBookRecord(bookRecord: ReaderRecordDomainModel) {
     ...
     * @return
     */
    fun getCollBook(): ReaderBookDomainModel {
        return mCollBook
    }
     ...
     * 获取当前的阅读记录
     */
    fun getRecord(): ReaderRecordDomainModel {
    ...
    protected abstract fun getChapterReader(chapter: ReaderChapterDomainModel): BufferedReader?

    /**
     * 章节数据是否存在
     */
    protected abstract fun hasChapterData(chapter: ReaderChapterDomainModel): Boolean
...
//    }
    ...
    private fun loadPages(
        chapter: ReaderChapterDomainModel,
        br: BufferedReader?
    ): MutableList<TxtPage> {
        //生成的页面
        ...
        val title: String = chapter.name//StringUtil.convertCC(chapter.name)
        var half: String
        try {

package com.woodnoisu.reader.feature.favourite.presentation.favourite.recyclerview
 ...
 * 书架适配器
 */
internal class ShelfAdapter : RecyclerView.Adapter<ShelfAdapter.ShelfViewHolder>() {

    // 上下文
    private var mContext: Context? = null

    //列表容器
    private val mList: MutableList<FavouriteBookDomainModel> =ArrayList()
    ...
    internal interface OnItemPositionClickListener{
        /**
         * 打开事件
         */
        fun openItem(position: Int, favouriteBookDomainModel: FavouriteBookDomainModel)

        /**
         * 删除事件
         */
        fun deleteItem(position: Int, favouriteBookDomainModel: FavouriteBookDomainModel)
    }

    internal class ShelfViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
     ...
     * 添加项目
     */
    fun addItem(value: FavouriteBookDomainModel?) {
        if(value!=null){
            mList.add(value)
            notifyDataSetChanged()
        }
     ...
     * 添加指定位置的项目
     */
    fun addItem(index: Int, value: FavouriteBookDomainModel) {
     ...
     * 批量添加项目
     */
    fun addItems(values: List<FavouriteBookDomainModel>) {
        mList.addAll(values)
        notifyDataSetChanged()
    }
     ...
     * 移除项目
     */
    fun removeItem(value: FavouriteBookDomainModel?) {
        mList.remove(value)
        notifyDataSetChanged()
    }
     ...
     * 批量移除项目
     */
    fun removeItems(values: List<FavouriteBookDomainModel>?) {
        if (values != null) {
            for (value in values) {
                mList.remove(value)
            }
     ...
     * 刷新容器
     */
    fun refreshItems(list: List<FavouriteBookDomainModel>?) {
        if (list != null) {
            mList.clear()
            mList.addAll(list)
            notifyDataSetChanged()
